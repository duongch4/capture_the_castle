...
void WinBackground::init(vec2 screen_size) {
    transform = Transform {
    ...
    // Load shared texture
    if (!winbgSprite.is_valid())
    {
        if (!winbgSprite.load_from_file(textures_path("ui/CaptureTheCastle_win_screen.png")))
        {
            fprintf(stderr, "Failed to load tile texture!");
        }
    }
    mesh.id = MeshManager::instance()->init_mesh(winbgSprite.width, winbgSprite.height);
}

void WinBackground::destroy() {
    effect.release();
    MeshManager::instance()->release(mesh.id);
}

void WinBackground::draw(const mat3 &projection) {
    ...
    glBindTexture(GL_TEXTURE_2D, winbgSprite.id);
...
vec2 WinBackground::get_position() {
    return vec2{transform.position.x, transform.position.y };
}

vec2 WinBackground::get_bounding_box() {
    return {(float)(fabs(transform.scale.x) * winbgSprite.width),
            (float)(fabs(transform.scale.y) * winbgSprite.height)};
}
